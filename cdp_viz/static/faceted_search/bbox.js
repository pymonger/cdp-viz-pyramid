...
(function () {
  ...
  */
  ...
  DragZoom.prototype.initControl_ = function (offset) {
    ...
    /* add info window - removed since we're using extjs to display help messages */
    /*
    var infoWindow = new google.maps.InfoWindow({
        content: "Click on the bbox control <img src='search_collections/dragzoom_btn_single.png'/>" +
                 " on the left to enter \"bbox draw mode\" then drag the crosshair cursor to draw the " +
                 "bounding box.",
        position: new google.maps.LatLng(-80, -20),
        maxWidth: 80,
        disableAutoPan: true
    });
  ...
  DragZoom.prototype.onMouseDown_ = function (e) {
    if (this.map_ && this.hotKeyDown_) {
      ...
      this.boxDiv_.style.visibility = "visible";
  ...
  DragZoom.prototype.onMouseMove_ = function (e) {
    ...
    if (this.dragging_) {
       ...
       * @event
       */
      var swPoint = new google.maps.Point(left, top + height);
      var nePoint = new google.maps.Point(left + width, top);
      google.maps.event.trigger(this, "drag", swPoint, nePoint, this.prjov_.getProjection());

      /* update text boxes */
      var prj = this.prjov_.getProjection();
      var sw = prj.fromContainerPixelToLatLng(swPoint);
      var ne = prj.fromContainerPixelToLatLng(nePoint);
      Ext.getCmp("sc_min_lat").setValue(sw.lat());
      Ext.getCmp("sc_max_lat").setValue(ne.lat());
      Ext.getCmp("sc_min_lon").setValue(sw.lng());
      Ext.getCmp("sc_max_lon").setValue(ne.lng());
  ...
  DragZoom.prototype.onMouseUp_ = function (e) {
    ...
    if (this.dragging_) {
      ...
      /* add event listeners for clearing out bbox */
      var bboxDiv = this.boxDiv_;
      google.maps.event.addListenerOnce(this.map_, 'click', function() {
        bboxDiv.style.visibility = "hidden";
      });
      google.maps.event.addListenerOnce(this.map_, 'zoom_changed', function() {
        bboxDiv.style.visibility = "hidden";
      });
    }
  };
